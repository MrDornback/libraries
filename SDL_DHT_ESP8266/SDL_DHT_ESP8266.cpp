/******************************************************************
  ...
  https://github.com/beegee-tokyo/arduino-SDL_DHT_ESP8266
...
void SDLdht::setup(uint8_t pin, DHT_MODEL_t model)
{
	SDLdht::pin = pin;
	SDLdht::model = model;
	SDLdht::resetTimer(); // Make sure we do read the sensor in the next readSensor()
...
void SDLdht::resetTimer()
{
	SDLdht::lastReadTime = millis() - 3000;
}

float SDLdht::getHumidity()
{
	readSensor();
	if (error == ERROR_TIMEOUT)
...
TempAndHumidity SDLdht::getTempAndHumidity()
{
	readSensor();
	if (error == ERROR_TIMEOUT)
	{ // Try a second time to read
...
const char *SDLdht::getStatusString()
{
	switch (error)
	{
	case SDLdht::ERROR_TIMEOUT:
		return "TIMEOUT";

	case SDLdht::ERROR_CHECKSUM:
		return "CHECKSUM";
...
float SDLdht::computeHeatIndex(float temperature, float percentHumidity, bool isFahrenheit)
{
...
//boolean isFahrenheit: True == Fahrenheit; False == Celcius
float SDLdht::computeDewPoint(float temperature, float percentHumidity, bool isFahrenheit)
{
...
//boolean isFahrenheit: True == Fahrenheit; False == Celcius
byte SDLdht::computePerception(float temperature, float percentHumidity, bool isFahrenheit)
{
...
//boolean isFahrenheit: True == Fahrenheit; False == Celcius
float SDLdht::getComfortRatio(ComfortState &destComfortStatus, float temperature, float percentHumidity, bool isFahrenheit)
{
	float ratio = 100; //100%
	float distance = 0;
...
float SDLdht::computeAbsoluteHumidity(float temperature, float percentHumidity, bool isFahrenheit)
{
